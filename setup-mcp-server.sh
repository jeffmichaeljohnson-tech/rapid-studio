...
# Rapid Studio MCP Server Setup Script
# Creates and configures the complete MCP development environment

set -e

echo "🚀 Setting up Rapid Studio MCP Server"
echo "===================================="

# Configuration
RAPID_STUDIO_ROOT="/Users/computer/rapid-studio-1"
MCP_DIR="$RAPID_STUDIO_ROOT/.mcp"
...
log "Creating MCP server directory structure..."
mkdir -p "$MCP_DIR"
mkdir -p "$RAPID_STUDIO_ROOT/.rapid-studio"

# Step 2: Install Python dependencies
log "Installing Python MCP dependencies..."
cd "$MCP_DIR"
...
log "Creating Rapid Studio MCP server..."
cat > rapid-studio-server.py << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
...
TAILSCALE_NETWORK = os.getenv("TAILSCALE_NETWORK", "100.103.213.111")
...
chmod +x rapid-studio-server.py
...
cat > "$CLAUDE_CONFIG" << EOF
{
  "mcpServers": {
    "rapid-studio-dev": {
      "command": "python3",
      "args": ["$MCP_DIR/rapid-studio-server.py"],
      "env": {
        "RAPID_STUDIO_ROOT": "$RAPID_STUDIO_ROOT",
        "RUNPOD_API_KEY": "rpa_1NNFJ0DJ15M6TSCW3D00FDO0QKS710LG0UKRSQ801rs5wv",
        "TAILSCALE_NETWORK": "100.103.213.111"
      }
    }
  }
}
...
log "Creating MCP server startup script..."
...
log "Creating MCP server test script..."

cat > "$MCP_DIR/test-server.sh" << 'EOF'
#!/bin/bash
...
timeout 10 python3 rapid-studio-server.py &
SERVER_PID=$!
...
chmod +x "$MCP_DIR/test-server.sh"

# Step 7: Create convenience scripts
log "Creating convenience scripts..."

# Project root helper script
cat > "$RAPID_STUDIO_ROOT/mcp-server" << EOF
#!/bin/bash
# Rapid Studio MCP Server Control Script
...
case "\$1" in
    ...
    test)
        ...
        ;;
    status)
        if pgrep -f "rapid-studio-server.py" > /dev/null; then
...
chmod +x "$RAPID_STUDIO_ROOT/mcp-server"

success "Setup completed successfully!"

echo ""
echo "🎉 RAPID STUDIO MCP SERVER SETUP COMPLETE!"
echo "=========================================="
echo ""
...
echo "2. Look for MCP server indicator in Claude chat"
echo "3. Test with: 'Use rapid_list_directory to show me the project structure'"
echo ""
echo "🛠️  Management Commands:"
echo "   ./mcp-server start   - Start MCP server manually"
echo "   ./mcp-server test    - Test MCP server"
echo "   ./mcp-server status  - Check server status"
echo ""
echo "🔧 Available MCP Tools:"
echo "   - rapid_read_file: Read any project file"
echo "   - rapid_write_file: Write/create files"
echo "   - rapid_list_directory: Browse directories"
echo "   - rapid_execute_script: Run scripts"
echo "   - rapid_deploy_gpu_fleet: Deploy Runpod GPUs"
echo "   - rapid_monitor_system: Monitor all services"
echo ""
echo "⚡ Ready for world-record development speed!"