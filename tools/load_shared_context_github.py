#!/usr/bin/env python3
"""
LOAD SHARED CONTEXT - GitHub-Integrated Auto-loading system
Automatically pulls from GitHub and loads shared context for both Cursor and Claude
"""
...
def auto_pull_from_github():
    ...
    print_section("🔄 AUTO-PULLING FROM GITHUB")
    
    try:
        # Check if we're in a git repository
        if not (RAPID_STUDIO / ".git").exists():
            print("❌ Not in a git repository")
            return False
        
        # Check git status
        status = run_command("git status --porcelain", cwd=RAPID_STUDIO)
        if status:
            print("⚠️  Local changes detected. Stashing before pull...")
            run_command("git stash", cwd=RAPID_STUDIO)
        
        # Pull latest changes
        print("📥 Pulling latest changes from GitHub...")
        result = run_command("git pull origin main", cwd=RAPID_STUDIO)
        if result:
            print(f"Git pull output: {result}")
        
        # Restore stashed changes if any
        if status:
            print("📤 Restoring local changes...")
            run_command("git stash pop", cwd=RAPID_STUDIO)
        ...
        print("⚠️  Continuing with local files only")
        return False
...
def main():
    ...
    ╔══════════════════════════════════════════════════════════════╗
    ║                                                              ║
    ║        LOAD SHARED CONTEXT - GITHUB-INTEGRATED SYSTEM      ║
    ║                                                              ║
    ╚══════════════════════════════════════════════════════════════╝
    """)
    
    try:
        # Auto-pull from GitHub first
        github_success = auto_pull_from_github()
        ...
        print_section("✅ GITHUB-INTEGRATED SHARED CONTEXT LOADED")
        print(f"""
🎯 Shared context loaded successfully with GitHub integration!
...
🚀 GitHub Integration:
   • {'✅ Auto-pulled from GitHub' if github_success else '⚠️  Using local files only'}
   • Always up-to-date with latest changes
   • Zero blindspots between agents
...
💡 Next steps:
   ...
   • Update shared context at session end
        ...
        print(f"\n❌ Error loading GitHub-integrated shared context: {e}")
        import traceback
        traceback.print_exc()
