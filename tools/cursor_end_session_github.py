#!/usr/bin/env python3
"""
CURSOR END OF SESSION - GitHub-Integrated Shared Context System
Writes to .ai-context/ AND auto-commits/pushes to GitHub for Claude synchronization
"""
...
def auto_commit_and_push(session_id):
    """Automatically commit and push to GitHub"""
    print_section("🚀 AUTO-COMMITTING TO GITHUB")
    
    try:
        # Add all .ai-context files
        print("📝 Adding .ai-context/ files to git...")
        result = run_command("git add .ai-context/", cwd=RAPID_STUDIO)
        if result:
            print(f"Git add output: {result}")
        
        # Commit with session info
        commit_message = f"Cursor session: {session_id} - Update shared context"
        print(f"💾 Committing: {commit_message}")
        result = run_command(f'git commit -m "{commit_message}"', cwd=RAPID_STUDIO)
        if result:
            print(f"Git commit output: {result}")
        
        # Push to GitHub
        print("🚀 Pushing to GitHub...")
        result = run_command("git push origin main", cwd=RAPID_STUDIO)
        if result:
            print(f"Git push output: {result}")
        
        print("✅ Successfully pushed to GitHub!")
        print("🎯 Claude can now see the latest Cursor session data!")
        
    except Exception as e:
        print(f"❌ Error during git operations: {e}")
        print("⚠️  Session data saved locally, but not pushed to GitHub")
...
def main():
    ...
    try:
        ...
        auto_commit_and_push(session_id)
        
        print_section("✅ CURSOR GITHUB-INTEGRATED SESSION COMPLETE")
        print(f"""
...
👋 Cursor session {session_id} ended with GitHub synchronization!
        """)
        
    except Exception as e:
        print(f"\n❌ Error during Cursor GitHub-integrated session end: {e}")
        import traceback
        traceback.print_exc()
